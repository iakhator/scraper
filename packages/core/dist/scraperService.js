    ...
    async init() {
        if (!this.cluster) {
            try {
                ...
                logger_1.logger.info('Puppeteer cluster initialized');
            }
            catch (error) {
    ...
    async close() {
        if (this.cluster) {
            try {
                ...
                this.cluster = null;
                logger_1.logger.info('Puppeteer cluster closed');
            }
            catch (error) {
        ...
        return this.cluster.execute(async ({ page }) => {
            ...
            try {
                ...
                if (error.name === 'TimeoutError') {
                    logger_1.logger.warn(`Timeout while loading ${url}, falling back to partial content`);
