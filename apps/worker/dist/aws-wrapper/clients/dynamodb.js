...
async function putItem(tableName, item, options) {
    ...
    try {
        ...
        logger_1.default.debug("DynamoDB putItem success", { tableName, itemId: item.id });
        ...
        logger_1.default.error("DynamoDB putItem failed", { error: err, tableName, itemId: item.id });
        return { error: err };
    }
}
async function getItem(tableName, key, options) {
    ...
    try {
        ...
        if (!Item) {
            logger_1.default.debug("DynamoDB getItem not found", { tableName, key });
            return { data: undefined };
        }
        logger_1.default.debug("DynamoDB getItem success", { tableName, key });
        return { data: (0, util_dynamodb_1.unmarshall)(Item) };
    }
    catch (error) {
        const err = handleDynamoError(error, "getItem", { tableName, key });
        logger_1.default.error("DynamoDB getItem failed", { error: err, tableName, key });
        return { error: err };
    }
}
async function updateItem(tableName, key, updateExpression, expressionAttributeValues, options) {
    ...
    try {
        ...
        logger_1.default.debug("DynamoDB updateItem success", { tableName, key });
        return { data: Attributes ? (0, util_dynamodb_1.unmarshall)(Attributes) : undefined };
    }
    catch (error) {
        const err = handleDynamoError(error, "updateItem", { tableName, key });
        logger_1.default.error("DynamoDB updateItem failed", { error: err, tableName, key });
        return { error: err };
    }
}
async function deleteItem(tableName, key, options) {
    ...
    try {
        ...
        logger_1.default.debug("DynamoDB deleteItem success", { tableName, key });
        return { data: Attributes ? (0, util_dynamodb_1.unmarshall)(Attributes) : undefined };
    }
    catch (error) {
        const err = handleDynamoError(error, "deleteItem", { tableName, key });
        logger_1.default.error("DynamoDB deleteItem failed", { error: err, tableName, key });
        return { error: err };
    }
}
